...
//1. 파셜클래스 키워드 추가 -> 생성기 돌릴 때 ~Data 파일에 메서드를 추가로 적을 때 리셋되는 거 방지.
// 근데 이거 그냥 원래 확장 메서드로 커버했는데 굳이?
...
public static class TableClassGenerator
{
    ...
    /// <summary>
    /// 지정된 폴더 내의 모든 TextAsset(CSV) 파일을 읽어 클래스 코드를 생성하고 저장
    /// </summary>
    ...
    {
        //폴더 내 모든 TextAsset 파일의 Asset Path 목록을 가져옴.
    ...
    /// <summary>
    /// CSV 파일을 읽어 클래스 생성에 필요한 데이터 구조를 반환
    /// </summary>
    ...
    /// <summary>
    /// CsvClassData를 기반으로 C# 클래스 코드를 생성
    /// </summary>
    public static string GenerateClassCode(CsvClassData data)
    {
        ...
        //ITableData 상속
        sb.AppendLine($"public class {className} : ITableData");
        sb.AppendLine("{");

        //첫 번째 컬럼을 ID로 사용하기 위해 저장
        string firstColumnName = data.Columns.Count > 0 ? data.Columns[0].Name : "";

        foreach (var col in data.Columns)
        {
            sb.AppendLine($"    public {col.Type} {col.Name};");
        }

        sb.AppendLine();

        //ITableData 인터페이스의 PrimaryID 구현
        if (!string.IsNullOrEmpty(firstColumnName))
        {
            sb.AppendLine($"    public string PrimaryID => {firstColumnName}.ToString();");
        }
        else
        {
            sb.AppendLine($"    public string PrimaryID => \"\";");
        }
